...
import (
	...
	"time"
	"math/rand"
	dhcp "github.com/lflxp/arm/udp"
	"log"
	"fmt"
)

func DhcpServer() {
	serverIp := net.IP{192,168,1,102}
	handler := &DHCPHandler{
		ip:		serverIp,
		leaseDuration:	2*time.Hour,
		start: 		net.IP{192,168,1,120},
		leaseRange:	50,
...
func (h *DHCPHandler) ServeDHCP(p dhcp.Packet, msgType dhcp.MessageType, options dhcp.Options) (d dhcp.Packet) {
	...
	case dhcp.Discover:
		fmt.Printf("Discover:%v",p.CIAddr())
	...
	case dhcp.Request:
		fmt.Printf("Request:%v",p.Options())
	...
	case dhcp.Release, dhcp.Decline:
		fmt.Println("release or Decline")
		nic := p.CHAddr().String()
